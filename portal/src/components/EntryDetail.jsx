...
export default function EntryDetail({ entry, allEntries = [], onClose, onDeleted, onNavigate, onUpdated, sharedCacheRef }) {
  ...
  const injectTimestamp = (html) => {
    ...
    let cleaned = html;
    while (/<div[^>]*>最后更新:[\s\S]*?<\/div>/g.test(cleaned)) {
      cleaned = cleaned.replace(/<div[^>]*>最后更新:[\s\S]*?<\/div>/g, '');
    }
    cleaned = cleaned.replace(/<div data-timestamp[^>]*>[\s\S]*?<\/div>/g, '');
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ padding: '6px 16px', borderBottom: '1px solid #2a2d35', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#161822', flexWrap: 'wrap', gap: 4 }}>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              ...
              {topicHTML && (
                <button onClick={() => {
                  const printCSS = `body, body > div, body > section, body > article, body > main, [class*="container"], [class*="wrapper"], [class*="content"] { max-width: 100% !important; width: 100% !important; margin-left: 0 !important; margin-right: 0 !important; padding-left: 20px !important; padding-right: 20px !important; } @media print { * { color: #111 !important; text-shadow: none !important; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; } body { background: #fff !important; } div, section, article, details, blockquote { border: 1px solid #d0d0d0 !important; border-radius: 6px !important; background: #f8f8f8 !important; margin-bottom: 8px !important; padding: 10px !important; overflow: visible !important; height: auto !important; max-height: none !important; max-width: 100% !important; } div > div, section > div { background: #fff !important; border-color: #e0e0e0 !important; } h1,h2,h3,h4,h5,h6 { background: transparent !important; border: none !important; page-break-after: avoid; } p, span, li, td, th, a, label, strong, em { background: transparent !important; border: none !important; } pre, code { background: #f0f0f0 !important; border: 1px solid #ccc !important; } pre, blockquote, table { page-break-inside: avoid; } img { max-width: 100% !important; } a { text-decoration: underline; } table { border-collapse: collapse !important; } td, th { border: 1px solid #ccc !important; padding: 6px !important; } }`;
